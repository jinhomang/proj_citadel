...
(function() {
	...
	Trex.Embeder.Media = Trex.Class.create({
        ...
        generateHTMLForDefaultEmbed: function (url, size) {
            return "<embed src=\"" + url + "\" width=\"" + size.width + "\" height=\"" + size.height + "\" " + this.getAllowScriptAccess(url) + "style=\"max-width: 100%; height: auto;\"></embed>";
        },
        generateHTMLForImage: function (url, size) {
            return "<img src=\"" + url + "\" border=\"0\" style= \"max-width: 100%; height: auto;\" />";
        },
        generateHTMLForFlash: function (url, size) {
            return "<embed src=\"" + url + "\" quality=\"high\" " + this.getAllowScriptAccess(url) + " type=\"application/x-shockwave-flash\" allowfullscreen=\"true\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\" wmode=\"transparent\" width=\"" + size.width + "\" height='" + size.height + "\" style=\"max-width: 100%; height: auto;\"></embed>";
        },
        generateHTMLForMoviePlayer: function (url, size) {
            return "<embed src=\"" + url + "\" type=\"application/x-mplayer2\" pluginspage=\"http://www.microsoft.com/Windows/MediaPlayer/\" width=\"" + size.width + "\" height=\"" + size.height + "\" style=\"max-width: 100%; height: auto;\"></embed>";
        },
        generateHTMLForQuicktime: function (url, size) {
            return "<embed src=\"" + url + "\" type=\"video/quicktime\" pluginspage=\"http://www.apple.com/quicktime/download/indext.html\" width=\"" + size.width + "\" height=\"" + size.height + "\" style=\"max-width: 100%; height: auto;\"></embed>";
        },
        generateHTMLIfIframeSource: function(url, size) {
            var tvpotKey = getTvPotKey(url);
            if (tvpotKey) {
                return "<iframe src=\"http://videofarm.daum.net/controller/video/viewer/Video.html?play_loc=undefined&vid=" + tvpotKey + "\" width=\""+size.width+"\" height=\""+size.height+"\" frameborder=\"0\" allowfullscreen style=\"max-width: 100%; height: auto;\"></iframe>";
            }

            var youtubeMovieKey = getYouTubeMovieKey(url);
            if (youtubeMovieKey) {
                return "<iframe src=\"http://www.youtube.com/embed/" + youtubeMovieKey + "\" width=\""+size.width+"\" height=\""+size.height+"\" frameborder=\"0\" allowfullscreen style=\"max-width: 100%; height: auto;\"></iframe>";
            }
